...
//    	addParallel(new );
    	...
    	addSequential(new DriveForDistanceBackward(2.5 * Robot.drivetrain.TICKS_PER_FOOT, 0.3, 0.0));
    	
    	if(!ready && Robot.intake.isLowLimitSwitchClicked()) {
    		ready = true;
    	} else if(!ready) {
    		addSequential(new DriveForDistanceBackward(0.1 * Robot.drivetrain.TICKS_PER_FOOT, -0.1, 0.0));
    	}
    	if(ready) {
